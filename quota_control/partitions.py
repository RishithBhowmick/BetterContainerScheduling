...
user_timeline_traces = requests.get("http://localhost:16686/api/traces?service=user-timeline-service&operation=read_user_timeline_server").json()
home_timeline_traces = requests.get("http://localhost:16686/api/traces?service=home-timeline-service&operation=read_home_timeline_server").json()
compose_post_traces = requests.get("http://localhost:16686/api/traces?service=compose-post-service").json()


user_timeline_trace = user_timeline_traces["data"][1]
home_timeline_trace = home_timeline_traces["data"][1]
compose_post_trace = compose_post_traces["data"][1]
...
def get_containers_in_service(service_name,container_list):        
   containers = [i.id for i in container_list if service_name in i.name or i.name=="socialnetwork-nginx-thrift-1"]
  #  print(containers)
   return containers
...
for key in dependencies.keys():
    ...
    all_partitions_with_ids[key] = list(set((chain.from_iterable(containers_for_service))))


pprint(all_partitions_with_ids)
